...
def build(output_dir: Path, figures_dir: Path) -> tuple[Path, Path]:
    ...
    readme = """# Anonymous reviewer code and evidence bundle\n\nThis snapshot contains the scientific files needed to evaluate the submitted ecological channel-identification boundary paper. The separate RACH/NOV/RACH-SEQ manuscript and benchmark are not included. Public repository metadata, author metadata and Git commit identifiers are also excluded.\n\nThe main theory sources are under `causal_model/`; figure generators are under `figure_sources/`; direct theorem tests are under `tests/`; and `reference_notes/` contains the primary-literature measurement-architecture audit.\n\n## Minimal environment\n\n```bash\npython -m pip install -r requirements-review.txt\nPYTHONPATH=. python -m pytest --rootdir=. -q tests\n```\n\nThe supplied figures are under `figures/`. `bundle_manifest.json` records SHA-256 hashes for every file.\n"""
