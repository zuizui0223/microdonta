...
Historical protocol and policy keys are read unchanged for provenance. Displayed
labels use the active terminology of Mechanism-Resolving Observation Design.
"""
...
def load_rows(path: Path) -> tuple[dict, dict[tuple[str, int], dict]]:
    ...
    rows = {(str(row["policy"]), int(row["budget"])): row for row in payload["policy_budget_aggregate"]}
...
def nuisance_contrast(rows: dict[tuple[str, int], dict], *, budget: int = 4) -> tuple[float, float, float, float]:
    guided = float(rows[("rach_seq", budget)]["mean_distractors_selected_mean"])
    random = float(rows[("random_order", budget)]["mean_distractors_selected_mean"])
    if guided <= 0 or random <= 0:
        raise RuntimeError("nuisance-selection fold contrast requires positive means")
    return guided, random, random / guided, 1.0 - guided / random
...
def make_figure(result_path: Path, output_path: Path) -> Path:
    ...
    policies = [("rach_seq", "information-guided"), ("random_order", "random order")]
    metrics = [
        ...
        ("mean_distractors_selected", "Mechanism-independent nuisance measurements", "count"),
    ]
    ...
    for ax, (metric, title, kind) in zip(axes, metrics):
        ...
        ax.set_ylim(-0.03, 1.05) if kind == "fraction" else ax.set_ylim(bottom=-0.05)
        ax.grid(alpha=0.2)
    ...
    guided4, random4, ratio4, reduction4 = nuisance_contrast(rows, budget=4)
    axes[3].text(
        0.03, 0.96,
        "Budget 4\n"
        f"random / information-guided = {random4:.3f} / {guided4:.3f}\n"
        f"= {ratio4:.1f}×  ({100.0 * reduction4:.1f}% reduction)",
        transform=axes[3].transAxes, ha="left", va="top", fontsize=8.5,
    ...
    fig.suptitle(
        "Information-guided design selects resolving measurements under a limited budget\n"
