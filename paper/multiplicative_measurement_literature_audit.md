...
Schupp, Jordano & Gómez (2010) is the lead exhibit because it states `SDE = Quantity × Quality` explicitly, is outside pollination, and itself emphasises context dependence of the quality term. The pollination papers then show that the same rate-by-effectiveness structure arose independently in a second ecological literature.
...
**N1-N4 mapping**: This is the strongest positioning source because it shows both sides of the argument in one established framework: multiplicative decomposition is standard, and the conditional quality term can change with ecological context. N3/N4 and the bounded bridge therefore target a real transport problem rather than an artificial algebraic possibility.
...
> Multiplicative quantity-by-quality decompositions are already standard in ecological measurement. Seed dispersal effectiveness is explicitly defined as Quantity × Quality, with quality representing the probability that a dispersed seed ultimately recruits, while pollinator importance has repeatedly been defined as visitation frequency multiplied by per-visit effectiveness. Our results do not propose these decompositions as new biological laws. They identify the information boundary created when one component is observed only through a proxy whose conversion may change across the comparison.

## Claims not to make

- Do not say that every ecological fitness measure is multiplicative.
- Do not say that these papers establish the exact `F` and `E` interpretation used in every ecological application.
...
Use this audit to support one compact paragraph in the Introduction/Methods. The main novelty claim remains narrow:

1. rich net-only observation classes remain invariant under channel reallocation;
2. bounded proxy drift yields a sharp joint identified set and a breakdown point;
3. the boundary implies a concrete anchor-and-transport observation-design rule;
4. the joint-set geometry implies a reporting rule that forbids treating the two channel uncertainties as independent.
