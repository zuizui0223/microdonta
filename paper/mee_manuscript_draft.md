...
D = 4.58). All four rules pass V1 and V2; the two with a literature-attributed
mechanism (Bergmann, Allen) also pass V3, and the two genuinely unresolved rules
(Foster, Gloger) are validated on the confound and observation-design structure
only (Fig. 5).
...
unresolved, so V3 is not applicable). The panel spans a 2-way confound
(Bergmann, Allen), a 3-way confound the pairwise-edge layer would miss but the
degeneracy-based test still resolves (Foster), and a higher-dimensional case
(Gloger). Ground truth is the literature's directional structure, not nature's
