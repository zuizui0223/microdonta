...
`CA(C) → 1.00`, `CA(B) → 0.08`. The inference layer is simulator-agnostic — it
...
posterior probability P ≈ 0.10 — a near-arbitrary "winner". (B) RACH instead
reports causal degeneracy D_RACH ≈ 3.57 of K = 4, with CA(S2) ≈ CA(S3) ≈ 0.66:
the selfing-syndrome (S2) and island-isolation (S3) mechanisms are jointly
admissible and unresolved. (C) NOV ranks candidate future observations by
expected gain in resolvability. (D) Adding the NOV-recommended nectar-guide
magnitude (truth = S3) resolves the confound: D 3.57→2.38, R 0.11→0.40,
CA(S3) 0.67→0.85, CA(S2) 0.66→0.51. Generated by
...
confound is reproduced (D ≈ 3.58 of 4, CA(B) ≈ CA(C)); (B) a quantitative
magnitude observation resolves it (CA(C) → 1.00, CA(B) → 0.08). Generated by
...
observations and true states (r ≈ 0.77); EVSI is the full-predictive expectation,
hence the spread across specific truths. Generated by
