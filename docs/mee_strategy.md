...
### Experiment 2 — Generality (≥2 systems)  (DONE)

Implemented in `causal_model/synthetic_demo.py`. The RACH inference layer
(causal admissibility, degeneracy, resolvability, EVSI) is **simulator-agnostic**
— it consumes accepted (θ, s) draws and a switch list with no Campanula
dependency. The synthetic 4-switch system (mechanisms B and C drive a trait with
the *same ordinal trend* but *different magnitude*) reproduces the Campanula
result almost exactly:

```
              D_RACH/K   R_RACH        confound        after quantitative obs
Campanula     3.57/4     0.11 → 0.40   CA(S2)≈CA(S3)   CA(S3) 0.67→0.85, CA(S2) 0.66→0.51
Synthetic     3.58/4     0.10 → 0.40   CA(B)≈CA(C)     CA(C)  0.66→1.00, CA(B) 0.66→0.08
```

Same workflow, same signatures, two unrelated systems → the contribution is the
method, not the Campanula ABM. Campanula is presented honestly as a
*low-resolution* real case whose degeneracy diagnosis *motivates* the
NOV-recommended fieldwork; the synthetic system shows the machinery in a fully
transparent setting.

Figure: `python -m causal_model.synthetic_demo --figure outputs/mee/synthetic_demo.png`
