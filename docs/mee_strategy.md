...
Design:
1. Generate synthetic data from a known state with **S3 ON, S2 OFF** (isolation
   as common cause), under the current 2-gradient y_obs.
2. **ABC model choice**: report the posterior over switch combinations and the
   MAP / "best" single model. Expectation: it places substantial / overconfident
   mass on an S2-containing model (wrong mechanism) because S2 and S3 are
   indistinguishable on these two gradients.
3. **RACH**: report D_RACH / R_RACH and the per-switch verdict. Expectation:
   S2 and S3 are flagged *unresolved* (high degeneracy) — RACH refuses the false
   choice.
4. **NOV**: rank candidate observations; expectation: the neutral-genetic-structure
   observation (Fis/Fst, informative about S3 isolation but not S2 selfing) is
   top-ranked as the confound-breaker.
5. Add that observation → degeneracy drops, S3 resolves, S2 correctly rejected.

Deliverable: a single multi-panel figure (posterior-over-models vs. degeneracy
verdict vs. NOV ranking vs. post-measurement resolution).
