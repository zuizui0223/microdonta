...
Step D (to build): promote a quantitative magnitude observation (e.g. the
measured selfing-rate or Fis gradient magnitude, or nectar-guide quantification)
as an `absolute_summary` row whose value matches S3's magnitude; add it; show
CA_j(S3)↑, CA_j(S2)↓, D_RACH↓ — the confound resolves.
