...
A useful ecological limitation statement can be converted into an MROD problem only after it is made explicit:

```text
which mechanisms remain compatible?
-> which distinctions among them are unresolved?
-> which feasible candidate observations have predictive partitions for those distinctions?
-> which candidate maximizes current information value?
-> if every available candidate has zero or non-estimable value, report that the present measurement vocabulary cannot resolve the ambiguity
```

This turns `future work should collect more data` into a conditional observation decision without pretending that every limitation is resolvable by available measurements.
